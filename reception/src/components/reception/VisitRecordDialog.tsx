...
import { useEditReceptionDialog } from '@/hooks/useEditReceptionDialog';
...
const VisitRecordDialog: React.FC<VisitRecordDialogProps> = ({ open, onClose, reception, onConfirm }) => {
  ...
  const { editDialogOpen, editingReception, handleOpenEditDialog, handleCloseEditDialog, handleSaveEditDialog } = useEditReceptionDialog();
  ...
  return (
    <>
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-full">
          ...
          <DialogFooter className="flex justify-end space-x-2 mt-2 pb-0">
            <Button
              ...
              color="gray"
              onClick={() => {
                handleOpenEditDialog(reception);
              }}
              className="border-gray-500 text-gray-500 p-2"
            >
              編集
            </Button>
      ...
      <EditReceptionDialog open={editDialogOpen} onClose={handleCloseEditDialog} reception={editingReception} onSave={handleSaveEditDialog} />
    </>
  );
};
